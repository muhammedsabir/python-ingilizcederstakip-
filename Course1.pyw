    ...
    def yaziciyaGonder(self):
        ...
        html += ('<head><title>Report</title><style></style></head>'
                 '<body><table width="100%">'
                    '<td></td>'
                    '<tr><td><h1>Excellent English Course</h1></td></tr>'
                 '<td></td>'
                 '<td></td>'
                    '<tr><td>Ad Soyad </td><td> Kayıt Tarihi </br>  </td><td> Ücret </br> </td><td> Ders Günleri  </td><td>  Eğitim Saati</td><td> Başlangıç Saati</td><td> Bitiş Saati</td></tr>'
                 '<tr><td>{0}</td><td></br>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td><td>{6}</td></tr>'
                 '</table>'
...
#datetime.date.today()
        global grupKey
        grupKey = False
        if self.ui.radioGrup.isChecked():
            print("grup radio")
            grupKey = True            
...
class PrettyWidget(QWidget):
    ...
    def tableWrite(self):
        ...
        html +="</br>"
        html +="<h1>Excellent English Course</h1>"
        html +="</br>"
        html +="</br>"
        html +="<tr><td>Ad Soyad</td><td>Ücret</td><td>Kayıt Tarihi</td><td>Başlangıç Tarihi</td><td>Ders Günleri</td><td>Eğitim Saati</td><td>Başlangıç Saati</td><td>Bitiş Saati</td></tr>"
       


        while row < allRows:		
            #html += "<tr><td>" + self.table.item(row, 0).text() + "</td>"
            html += "<tr><td>" + self.table.item(row, 1).text() + "</td>"
            #html += "<td>" + self.table.item(row, 2).text() + "</td>"
            print("kayıt tirhi :"+self.table.item(row, 6).text())
